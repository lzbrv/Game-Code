...
- `railgun_fire.glb` — mesh + materials + two baked clips, **`Fire`** and **`Reload`**. Get it from the
...
| `mag_pivot` | the detachable power cell — drops 300 mm (30 units) on `Reload` |
| `railgun` | recoil: 45 mm back, −0.10 rad pitch |

`Fire` is **1.90 s** — charge 1.05 s, discharge 0.10 s, decay 0.75 s. The discharge frame
(t = 1.05 s) is where you spawn the beam / muzzle flash.

`Reload` is **0.80 s** — the cell drops out (0–0.26 s), the fresh one rides up (0.26–0.66 s)
and seats with a bump (0.66–0.80 s), matching the SMG's reload window so both weapons share
one timing. The cell is `power_cell` + `cell_cap` + `cell_cap_rear` + `cell_base`, all
parented to `mag_pivot`; `cell_well` stays on the frame.
